...
namespace AgglomerativeClustering
{
    public class AgglomerativeClustering
    {
        ...
        public List<BiCluster> Cluster(List<float[]> rows, Similarity similarity, int numNodes)
        {
            ...
            var clusters = new List<BiCluster>();
            for (int i = 0; i < rows.Count; i++)
                clusters.Add(new BiCluster(rows[i], id: i));

            Console.WriteLine("Starting Clusters: {0}", clusters.Count);

            // hard code size of grid for performance. If more than 20000 nodes then need to up this here.
            ...
            while (clusters.Count > 1)
            {
                ...
                BiCluster newCluster = CalculateAvgCluster(clusters, lowestPair, closest, currClusterId, numDims);
        ...
        private BiCluster CalculateAvgCluster(List<BiCluster> clusters, int[] lowestPair, double closest, int currClusterId, int numDims)
        {
            // we have radically different cluster sizes so halfing the distance won't work. Need to re-calculate centroid.
            var leafClusters1 = new List<BiCluster>();
            Tree.GetLeafClusters(clusters[lowestPair[0]], leafClusters1);
            var leafClusters2 = new List<BiCluster>();
            ...
            return newCluster;
                ...
                {
                    //if (i % 40 == 0) Console.WriteLine("{0}", i);
                    for (int j = (i + 1); j < clusters.Count; j++)
                    {
                        int row = clusters[i].Id + offset;
                        int col = clusters[j].Id + offset;

                        if (distances3[row][col] == -10.0d)
                        {
                            distances3[row][col] = (float) similarity.GetDistance(clusters[i], clusters[j]);
                            cacheMisses++;
                        }

                        double d = distances3[row][col];

                        if (d < closest)
                        {
                            closest = d;
                            lowestPair = new int[] { i, j };
                        }
                    }
                }
                // calculate the average of the two clusters. bit shitty as only works on centroids.
                // we have radically different cluster sizes so halfing the distance won't work. Need to re-calculate centroid.
                var leafClusters1 = new List<BiCluster>();
                Tree.GetLeafClusters(clusters[lowestPair[0]], leafClusters1);
                var leafClusters2 = new List<BiCluster>();
