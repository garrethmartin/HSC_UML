...
namespace NearestNeighbourLib
{
    ...
    public class PearsonAndDC : DistanceFunctions
    {
        private int numPowerSpectrums = -1;
        private int numValsInPowerSpectrum = -1;
        private DistanceFunctions pearson = null;
        ...
        public double Distance(double[] p1, double[] p2)
        {
            double distance = pearson.Distance(p1, p2);

            if (numValsInPowerSpectrum == -1)
                numValsInPowerSpectrum = p1.Length / numPowerSpectrums;

            int zeroDCIndex = 0;
            for (int i = 0; i < numPowerSpectrums; i++)
            {
                double diff = Math.Abs(p1[zeroDCIndex] - p2[zeroDCIndex]);
                if (diff > 0.2)
                    distance += 0.2;
