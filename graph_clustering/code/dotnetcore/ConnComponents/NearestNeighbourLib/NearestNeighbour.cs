...
/*
private static List<int[]> RunThreads(List<double[]> samples, KDTree.KDTree<int> tree, int[,] indexes, int numThreads)
{
    Task<List<int[]>>[] taskArray = new Task<List<int[]>>[numThreads];
    ...
    for (int iter = 0; iter < numThreads; iter++)
    {
        int tstart_idx = indexes[iter, 0]; // must copy local here see:
        int tend_idx = indexes[iter, 1]; // must copy local here see:
        taskArray[iter] = Task<List<int[]>>.Factory.StartNew(() => { return GetNearestNeighbours(samples, tree, tstart_idx, tend_idx); });
    }
 ...
 * 
 *         /*
public static KDTree.KDTree<int> CreateKDTree(List<double[]> nodePositions)
{
    int numDims = nodePositions[0].Length;
    var pTree = new KDTree.KDTree<int>(numDims);
...
public static List<int[]> GetNearestNeighbours(List<double[]> samples, KDTree.KDTree<int> tree, int numThreads)
{
    int numDim = samples[0].Length;
    int numSamples = samples.Count;

    if (numThreads == 1 || numSamples < 10000)
    {
        return GetNearestNeighbours(samples, tree, 0, numSamples);
    }
    else
    {
        int[,] indexes = GetIndexes(samples.Count, numThreads);
        List<int[]> results = RunThreads(samples, tree, indexes, numThreads);
        results.Sort(new IntComp()); // sort to make sure are in the order of the index and not the order they were threaded
        return results;
    }
}

private static List<int[]> GetNearestNeighbours(List<double[]> samples, KDTree.KDTree<int> tree, int startIdx, int endIdx)
{
    List<int[]> nodeIndex = new List<int[]>();

    for (int i = startIdx; i < endIdx; i++)
    {
        KDTreeNearestNeighbour<int> nn = tree.NearestNeighbors(samples[i], 1);
