...
{
    public class NearestNeighbourVQ
    {
        static int ACTIVE = 1;
        public enum Metric { Euclidean, Cosine };

        public static double[,] GetTwoNearestNeighbours(int metric, int num_threads, int num_nodes, float[] sample, List<float[]> data)
        {
            int numDim = sample.Length;

            if (num_threads == 1 || num_nodes < 2)
            {
                switch (metric)
                {
                    case 1: // euclidean
                        return FindTwoNNEuclidean(numDim, sample, data, 0, num_nodes);
                    case 2: // cosine
                        return FindTwoNNCosine(numDim, sample, data, 0, num_nodes);
                    case 3: // pearson
                        return FindTwoNNPearson(numDim, sample, data, 0, num_nodes);
                    default:
                        return FindTwoNNEuclidean(numDim, sample, data, 0, num_nodes);
                }                
            }
            else
            {
                ...
                List<double[,]> threadResults = RunThreads(metric, num_threads, numDim, indexes, sample, data);
        ...
        public static List<double[,]> RunThreads(int metric, int num_threads, int num_dim, int[,] indexes, float[] sample, List<float[]> data)
        {
        ...
        public static double[,] FindTwoNNCosine(int ndim, float[] a, List<float[]> data, int start_idx, int end_idx)
        {
            double[,] results = new double[3, 2];
        ...
        public static double[,] FindTwoNNPearson(int ndim, float[] a, List<float[]> data, int start_idx, int end_idx)
        {
            double[,] results = new double[3, 2];
            ...
            for (job = start_idx; job < end_idx; job++)
            {
                ...
                for (int i = 0; i < 3; i++)
                {
                    double diff = Math.Abs(a[zeroDCIndex] - data[job][zeroDCIndex]);
                    if (diff > 0.2)
                        dist += 0.2;
        ...
        public static double[,] FindTwoNNEuclidean(int ndim, float[] a, List<float[]> data, int start_idx, int end_idx)
        {
            double[,] results = new double[3, 2];
