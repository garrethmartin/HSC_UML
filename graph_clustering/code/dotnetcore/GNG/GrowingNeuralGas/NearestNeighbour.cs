...
namespace GNG
{
    public class NearestNeighbour
    {
        ...
        public static double[,] GetTwoNearestNeighbours(int metric, int num_threads, int num_nodes, int num_dim, float[] sample, float[][] data)
        {
            if (num_threads == 1 || num_nodes < 2)
            {
                switch (metric)
                {
                    ...
                    case 4: // Fractional Jim
                        return FindTwoNNFractionalEuclidean(num_dim, sample, data, 0, num_nodes);
                    default:
                        return FindTwoNNEuclidean(num_dim, sample, data, 0, num_nodes);
                }                
            }
            ...
            for (int iter = 0; iter < num_threads; iter++)
            {
                ...
                switch(metric)
                {
                    ...
                    case 3:
                        ...
                        break;
                    case 4:
                        taskArray[iter] = Task<double[,]>.Factory.StartNew(() => { return FindTwoNNFractionalEuclidean(num_dim, sample, data, tstart_idx, tend_idx); });
                        break;
        ...
        public static double[,] FindTwoNNFractionalEuclidean(int ndim, float[] a, float[][] data, int start_idx, int end_idx)
        {
            double[,] results = new double[3, 2];
            ...
            for (job = start_idx; job < end_idx; job++)
            {
                ...
                {
                    double numerator = (a[i] - data[job][i]);
                    numerator = numerator * numerator;

                    double denominator = (0.5 * (a[i] + data[job][i]));
                    denominator = denominator * denominator;

                    total += numerator / denominator;
                }

                dist = total; //sim.distance(a, data[job]);
                //
